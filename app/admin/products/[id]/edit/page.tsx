        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white border border-border rounded-xl p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="brand" value={form.brand} onChange={handleChange} className="w-full border border-border rounded-lg px-3 py-2.5 text-sm outline-none focus:border-primary bg-white" />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-muted mb-1.5">SKU</label>
                  <input name="sku" value={form.sku} onChange={handleChange} className="w-full border border-border rounded-lg px-3 py-2.5 text-sm outline-none focus:border-primary bg-white" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-muted mb-1.5">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  className="w-full border border-border rounded-lg px-3 py-2.5 text-sm outline-none focus:border-primary resize-none bg-white"
                />
              </div>
            </div>
                ...
                {features.map((f, i) => (
                  <div key={i} className="flex gap-2">
                    <input
                      ...
                      className="flex-1 border border-border rounded-lg px-3 py-2 text-sm outline-none focus:border-primary bg-white"
                      placeholder={`Feature ${i + 1}`}
                    />
                    <button
                      type="button"
                      onClick={() => setFeatures(fs => fs.filter((_, j) => j !== i))}
                      className="w-8 h-9 flex items-center justify-center text-danger hover:bg-red-50 rounded-lg text-lg"
                    >
                      ×
                    </button>
                  </div>
          ...
          <div className="space-y-5">
            <div className="bg-white border border-border rounded-xl p-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-muted mb-1.5">Old Price (KES)</label>
                <input
                  name="old_price"
                  type="number"
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-muted mb-1.5">Weight (kg)</label>
                <input
                  name="weight_kg"
                  type="number"
                  step="0.1"
            ...
            <div className="bg-white border border-border rounded-xl p-5">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {images.map((url, i) => (
                    <div key={i} className="relative w-16 h-16 rounded-lg overflow-hidden border border-border group">
                      ...
                      {i === 0 && (
                        <span className="absolute bottom-0 inset-x-0 bg-primary text-white text-2xs text-center py-0.5">Main</span>
                      )}
                      <button
                        type="button"
                        onClick={() => setImages(imgs => imgs.filter((_, j) => j !== i))}
                        className="absolute top-0.5 right-0.5 w-4 h-4 bg-danger rounded-full text-white text-2xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        ×
                      </button>
                    </div>
              ...
              <label
                className={`block border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-colors ${
                  uploading ? 'border-primary bg-orange-50' : 'border-border hover:border-primary'
                }`}
              >
