...
async function uploadBannerImage(e: React.ChangeEvent<HTMLInputElement>) {
  ...
  const compressed = await compressImage(file, 'HERO_BANNER')
  const path = `hero/${Date.now()}-${compressed.name.replace(/\s+/g, '-')}`
...
async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
  ...
  const compressed = await compressImage(file, 'CATEGORY_PHOTO')
  ...
  await supabaseBrowser
    .from('site_settings')
    .upsert({ key: `category_image_${slug}`, value: url }, { onConflict: 'key' })
