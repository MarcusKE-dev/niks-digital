...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  // Get current path to exclude login page from auth check
  ...
  // Do NOT protect the login page - let middleware handle it
  if (!user && !pathname.includes('/admin/login')) {
    redirect('/admin/login')
  }

  // If user is logged in and is trying to access login page, redirect to dashboard
  if (user && pathname.includes('/admin/login')) {
    redirect('/admin')
  }
    ...
    <div className="min-h-screen flex bg-surface">
      ...
      <aside className="w-56 bg-dark-500 flex flex-col flex-shrink-0">
        ...
        <div className="p-4 border-t border-white/10">
          <p className="text-xs text-white/30 mb-2 truncate">{user?.email || 'Admin'}</p>
